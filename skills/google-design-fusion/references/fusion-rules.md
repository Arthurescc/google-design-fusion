# Fusion Rules

## Core rule

Do not average references. Build a directed mix.

## Recommended recipe

1. Pick one Google Design principle cluster:
   attention, motion, typography, color, accessibility, AI trust, ambient computing, or Material expressiveness.
2. Pick two strong `awesome-design-md` seeds:
   one base sample and one contrast sample.
3. Optionally add one stretch sample:
   only if it sharpens the thesis.

## Translate into explicit axes

Define these axes before writing UI:

- brand archetype
- atmosphere keywords
- color strategy
- typography signature
- geometry signature
- depth strategy
- layout rhythm
- density profile
- component signatures
- imagery strategy
- motion behavior

If these axes are not explicit, the output is still too vague.

## Guardrails

- Never blend more than three strong visual sources.
- Never let `awesome-design-md` override accessibility, clarity, or semantic hierarchy.
- Never use Google Design principles as an excuse for bland visual output.
- Never keep a sample just because it looks fashionable; keep it only if it reinforces the thesis.

## Good examples

- Google Design attention + ambient motion
  paired with Notion warmth + Stripe precision
- Material expressiveness
  paired with Apple restraint + Spotify depth
- AI clarity and trust
  paired with Linear density + Vercel discipline
